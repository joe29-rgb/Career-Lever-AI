...
export function TopNav() {
  ...
  return (
    <nav className="sticky top-0 z-40 bg-white/90 dark:bg-gray-900/80 backdrop-blur border-b">
        ...
        <div className="px-4 py-3 grid gap-2 text-sm">
          {LINKS.map(link => (
            <Link
              key={link.href}
              href={link.href}
              className={`block px-3 py-2 rounded ${isActive(link.href) ? 'bg-blue-50 text-blue-700 dark:bg-blue-950/30 dark:text-blue-300' : 'text-gray-800 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800'}`}
            >
              {link.label}
            </Link>
          ))}
